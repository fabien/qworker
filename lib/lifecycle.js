...
var defaultTypes = ['initializer', 'finalizer', 'terminator', 'observer'];
...
Lifecycle.prototype._executeHandler = function(type, context, handler, callback) {
    ...
    if (handler.fn.length === 2) { // async
        handler.fn.call(null, ctx, function(err, result) {
            this._executedHandler(type, ctx, result, err);
            if (_.isFunction(callback)) callback(err, result, ctx);
        }.bind(this));
    } else {
        var result = handler.fn.call(null, ctx);
